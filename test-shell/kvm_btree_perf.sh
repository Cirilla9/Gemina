...
# 定义输出文件路径
output_directory="./$kernel_name/btree/${current_date}_${current_time}"
...
for vm_name in $vm_list; do
  ...
  if [[ $vm_name =~ ^Ubuntu[0-9]+$ ]]; then
    ...
    echo "run btree..."
    ssh zz@$vm_ip 'echo "123456" | sudo -S sh -c "echo always > /sys/kernel/mm/transparent_hugepage/enabled; echo always > /sys/kernel/mm/transparent_hugepage/defrag"'
    # 在宿主机上使用ssh连接到虚拟机，并将负载命令放入后台执行
    ssh zz@$vm_ip "cd /home/zz/benchmarks/btree ; ./BTree" > "${output_directory}/${vm_name}.txt" &
...
for vm_name in $vm_list; do
  ...
  if [[ $vm_name =~ ^Ubuntu[0-9]+$ ]]; then
    ...
    # 从输出文件中提取属性的值
    teps_value=$(grep "$teps_metric" "$output_file" | awk '{print $5}')
    # 检查是否成功获取到值
