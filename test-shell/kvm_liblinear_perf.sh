...
# 定义输出文件路径
output_directory="./$kernel_name/liblinear/${current_date}_${current_time}"
...
for vm_name in $vm_list; do
  ...
  if [[ $vm_name =~ ^Ubuntu[0-9]+$ ]]; then
    ...
    ssh zz@$vm_ip "cd /home/zz/benchmarks/liblinear ; /usr/bin/time -f '%s' -p ./train -s 1 kdda" > "${output_directory}/${vm_name}.txt" '2>&1' &
...
for vm_name in $vm_list; do
  ...
  if [[ $vm_name =~ ^Ubuntu[0-9]+$ ]]; then
    ...
    # 从输出文件中提取属性的值
    teps_value=$(grep "$teps_metric" "$output_file" | awk '{split($2, a, "h"); split(a[length(a)], b, "m"); split(b[length(b)], c, "s"); time = 0; if (length(a) > 1) time += a[1] * 3600; if (length(b) > 1) time += b[1] * 60; time += c[1]; print time}')
    # 检查是否成功获取到值
