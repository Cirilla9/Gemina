...
# 定义输出文件路径
output_directory="./$kernel_name/npb/${current_date}_${current_time}"
...
for vm_name in $vm_list; do
  ...
  if [[ $vm_name =~ ^Ubuntu[0-9]+$ ]]; then
    ...
    ssh zz@$vm_ip "cd /home/zz/benchmarks/NPB3.4-OMP/bin ; ./cg.D.x" > "${output_directory}/${vm_name}.txt" '2>&1' &
