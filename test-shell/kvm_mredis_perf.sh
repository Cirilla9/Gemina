...
# 定义输出文件路径
output_directory="./$kernel_name/mredis/${current_date}_${current_time}"
...
for vm_name in $vm_list; do
  ...
  if [[ $vm_name =~ ^Ubuntu[0-9]+$ ]]; then
    ...
    ssh zz@$vm_ip "memtier_benchmark --server=127.0.0.1 --port=6379 --protocol=redis --threads=4 --clients=50 --data-size=1024 --key-pattern=P:P --print-percentiles 90,99,99.9,99.99 --key-maximum=1000000 --ratio=1:1 --test-time=600 --pipeline=10" > "${output_directory}/${vm_name}.txt" &
